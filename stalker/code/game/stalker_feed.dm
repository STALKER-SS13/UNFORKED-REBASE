...
/datum/data/stalker_feed_message
	...
	var/message
	var/is_faction_restricted = FALSE

/datum/data/stalker_feed_message/New(datum/record/stalker/sender, message, is_faction_restricted)
	src.sender = sender
	src.message = message
	src.is_faction_restricted = is_faction_restricted

/proc/add_stalker_feed_message(datum/record/stalker/sender, message, is_faction_restricted)
	ASSERT(istype(sender))
	var/datum/data/stalker_feed_message/new_message = new(sender, message, is_faction_restricted)
...
/proc/format_stalker_feed_message(datum/data/stalker_feed_message/message)
	...
	<p class='lentamsg'><b><font color = '[get_faction_color(message.sender.stalker_faction)]'>[sender.name]\[[sender.stalker_faction][sender.is_faction_leader ? " - leader" : ""]\][message.is_faction_restricted ? "(faction chat)" : ""]</font></b> : <br>
...
/proc/show_dead_lenta_message(datum/data/stalker_feed_message/message)
	for(var/mob/dead/observer/M in GLOB.player_list)
		to_chat(M, "STALKER FEED: <font color='[get_faction_color(message.sender.stalker_faction)]'>[message.sender.name]\[[message.sender.stalker_faction]\][message.is_faction_restricted ? "(faction chat)" : ""]:</font>[message.message]")
